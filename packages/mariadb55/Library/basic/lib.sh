...
# ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
#
#   lib.sh of /CoreOS/mariadb55/Library/basic
...
mariadb55/basic - set of basic functions for MariaDB
...
mariadbStart() {
    if ! [[ -e $mariadbLog ]] ; then
       rlLogInfo "Restoring the log file (workaround bug BZ#832035)"
...
mariadbLibraryLoaded() {
    if rpm=$(rpm -qa rh-mariadb*-mariadb || rpm -q mariadb55 || rpm -q mariadb); then
        ...
        mariadbVersion=`rpm -q --qf %{VERSION} ${PACKAGE}-server`
    ...
    case "$SYSPATHS" in
        *rh-mariadb102)
        ...
        rlFail "Package mariadb[55|10x] is not installed."
